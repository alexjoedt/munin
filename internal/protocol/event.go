...
// PublishEvent is the payload enveloped in a [TypePublish] Message.
// Wire format:
//
...
func NewPublishEvent(topic string, data []byte) *PublishEvent {
	return &PublishEvent{
		ID:    xid.New(),
		Topic: topic,
		Data:  data,
...
// MarshalWire marshals [PublishEvent] into a binary format.
func (e *PublishEvent) MarshalWire() ([]byte, error) {
	topic := []byte(e.Topic)
...
func (e *PublishEvent) UnmarshalWire(b []byte) error {
	offset := 0
